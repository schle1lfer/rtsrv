 ...
 * @code
 *   void on_route(netlink_event_t ev,
 *                 const netlink_route32_t *r, void *ud)
 *   {
 *       const char *op = (ev == NETLINK_ROUTE_ADDED) ? "ADD"
 *                      : (ev == NETLINK_ROUTE_CHANGED) ? "CHG" : "DEL";
 *       printf("%s %s gw=%s dev=%s metric=%u\n",
 *              op, inet_ntoa(r->dst), inet_ntoa(r->gateway),
 *              r->ifname, r->metric);
 *   }
 *   // ...
 *   int fd = netlink_init();
 *   if (fd < 0) { perror("netlink_init"); return; }
 *   netlink_run(fd, on_route, nullptr);
 *   netlink_close(fd);
 * @endcode
 *
 * @version 1.0
...
#include <stdint.h>
     ...
     *                           An existing route replaced in-place; the
     *                           new descriptor carries updated attributes
     *                           (gateway, metric, etc.).
     *                           Triggered by "ip route replace/change".
     *                           FRR zebra uses this for nexthop updates.
     *
    ...
    typedef enum
    {
        ...
        NETLINK_ROUTE_CHANGED = 2, /**< Route attributes updated in-place. */
    } netlink_event_t;
