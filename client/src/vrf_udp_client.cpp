...
void VrfUdpClient::run()
{
    ...
    std::println("[vrf_udp_client] starting: socket={} vrf={} nexthop={}",
                 cfg_.socket_path, cfg_.vrfs_name, fmt_ip(cfg_.nexthop_addr));
    ...
    auto cmd_bytes = cmdproto::encode_command(cmdproto::make_route_add_vrfs(req));
    if (!cmd_bytes)
    {
        std::println(stderr, "[vrf_udp_client] encode_command: {}",
    ...
    if (!exch_bytes)
    {
        std::println(stderr, "[vrf_udp_client] encode_exchange: {}",
                     exch_bytes.error().message());
        running_.store(false);
        return;
    }

    auto data_msg  = routeproto::make_data(msg_id,
                         std::span<const std::uint8_t>{*exch_bytes});
    auto msg_bytes = routeproto::encode_message(data_msg);
    if (!msg_bytes)
    {
        std::println(stderr, "[vrf_udp_client] encode_message: {}",
    ...
    if (!frame)
    {
        std::println(stderr, "[vrf_udp_client] encode_packet: {}",
                     frame.error().message());
        running_.store(false);
        return;
    }
    ...
    if (!sock_result)
    {
        std::println(stderr, "[vrf_udp_client] create_socket: {}",
    ...
    {
        std::println(stderr, "[vrf_udp_client] connect_socket({}): {}",
                     cfg_.socket_path, r.error().message());
        running_.store(false);
        return;
    }
    ...
    {
        std::println(stderr, "[vrf_udp_client] send_all: {}",
                     r.error().message());
        running_.store(false);
        return;
    }
    ...
    if (!frame_recv)
    {
        std::println(stderr, "[vrf_udp_client] recv_frame: {}",
    ...
    if (!reply_pkt)
    {
        std::println(stderr, "[vrf_udp_client] decode_packet: {}",
    ...
    if (!reply_msg)
    {
        std::println(stderr, "[vrf_udp_client] decode_message: {}",
    ...
    if (!vrf_resp)
    {
        std::println(stderr, "[vrf_udp_client] decode_vrfs_route_response: {}",
