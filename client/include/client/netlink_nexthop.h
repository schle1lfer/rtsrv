...
#include <stdint.h>
     ...
     *
     * - NETLINK_NEXTHOP_ADDED   – RTM_NEWNEXTHOP without NLM_F_REPLACE, or a
     * dump response.
...
#define NETLINK_NEXTHOP_MAX_GROUP 64
    ...
    typedef struct
    {
        ...
        /* ── NHA_GATEWAY ─────────────────────────────────────────────────────
         */
        char gateway[INET6_ADDRSTRLEN]; /**< Gateway address string ("" if
                                           absent) */
         ...
         */
        netlink_nexthop_grp_t
            group[NETLINK_NEXTHOP_MAX_GROUP]; /**< Group members */
     ...
     * the RTNLGRP_NEXTHOP multicast group via NETLINK_ADD_MEMBERSHIP.
     *
     * Nexthop objects were added in Linux 5.3.  On older kernels the socket
     * will open successfully but no RTM_NEWNEXTHOP / RTM_DELNEXTHOP events will
     * arrive and the dump will return 0 entries.
     *
     ...
     * out.
     *
     * Call after netlink_nexthop_dump() to receive ongoing nexthop change
     * events. Returns 0 on clean close, -1 on error.
     *
     * @param fd        File descriptor from netlink_nexthop_init().
